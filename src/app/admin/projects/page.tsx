...
export default function AdminProjectsPage() {
  ...
  useEffect(() => {
    const verifyAuthentication = async () => {
      ...
      try {
        console.log('[PROJECTS] Verifying authentication with server');
        ...
        console.log('[PROJECTS] Verify response status:', verifyResponse.status);
        if (!verifyResponse.ok) {
          console.log('[PROJECTS] Server verification failed, redirecting to login');
          router.push("/admin/login");
          return;
        }

        const verifyData = await verifyResponse.json();
        console.log('[PROJECTS] Server verification successful for user:', verifyData.user?.email);
        setIsAuthenticated(true);

        // Fetch projects after authentication
        fetchProjects();
      } catch (error) {
        console.error('[PROJECTS] Authentication verification error:', error);
        router.push("/admin/login");
      } finally {
        setIsLoading(false);
      }
